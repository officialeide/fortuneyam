...
// 팔괘(오행→대표 괘)
const TRIGRAM={水:"감(坎)",木:"진(震)",火:"이(離)",土:"곤(坤)",金:"건(乾)"};
// 토정비결 사자성어 (상괘 1~8 기준)
const TOJUNG_SAJA={
...
const ILGAN_DESC = {
  갑: {
    ...
    night: {
      ...
      idealType2: "나의 결과를 진심으로 기뻐해주는 사람이 실제 궁합이에요. 화(火) 일간 — 병화(丙火), 정화(丁火) — 이 잘 맞아요. 갑목(甲木)이 불을 키우는 목생화(木生火) 구조로, 내가 에너지를 주고 상대가 그 열정을 빛으로 펼치는 아름다운 상생이에요."
    }
  },
  을: {
    ...
    night: {
      desire: "사주 글자 안에 숨겨진 기운: 을목(乙木)은 신금(辛金)과 乙辛충(충)의 긴장을 가지고 있어요. 상처를 쉽게 받지만 그 상처가 오히려 감수성을 더 깊게 만들어요. '나의 세계를 있는 그대로 인정받고 싶다, 강요 없이 내 페이스를 존중받고 싶다'는 욕구가 내면 깊숙이 있어요. 관계 속에서도 나를 잃고 싶지 않은 마음이 강해요.",
      desire2: "목(木) 일간의 재성 구조: 토(土)가 재성이에요. 을목(乙木)은 부드럽게 끌려드는 합(合)의 에너지를 가지고 있어요. 강요 없이 자연스럽게 이어지는 관계를 원하고, 억지로 맞추는 관계는 본능적으로 불편해요.",
      triggers: [
      ...
      idealType2: "안정적이고 든든하게 받쳐주는 사람이 실제 궁합이에요. 수(水) 일간 — 임수(壬水), 계수(癸水) — 이 잘 맞아요. 수생목(水生木), 을목(乙木)을 키워주는 에너지예요. 나의 유연함을 꺾지 않으면서 깊은 곳에서 지지해주는 파트너예요."
    }
  },
  병: {
    ...
    night: {
      ...
      desire2: "화(火) 일간의 재성 구조: 금(金)이 재성이에요. 병화(丙火)는 금(金)을 녹여서 형태를 만들어요. 관계에서도 상대를 나의 에너지로 감싸고 변화시키는 것을 좋아해요. 내가 먼저 다가가지만 진심은 천천히 꺼내요.",
      triggers: [
      ...
      idealType2: "안정적이고 깊이 있는 사람이 실제 궁합이에요. 목(木) 일간 — 갑목(甲木), 을목(乙木) — 이 잘 맞아요. 목생화(木生火), 나의 불을 키워주는 에너지예요. 내가 타오를 수 있도록 조용히 연료가 되어주는 파트너예요."
    }
  },
  정: {
    ...
    night: {
      ...
      idealType2: "나의 조용함을 편하게 여기는 사람이 실제 궁합이에요. 목(木) 일간 — 갑목(甲木), 을목(乙木) — 이 잘 맞아요. 목생화(木生火), 나의 불을 오래 피울 수 있게 도와주는 에너지예요. 시끄럽지 않아도 함께 있는 게 편한 관계를 원해요."
    }
  },
  무: {
    ...
    night: {
      desire: "사주 글자 안에 숨겨진 기운: 술토(戌土) 속에는 정화(丁火)와 신금(辛金)이 잠들어 있어요. 화(火) 기운이 없는 사주에서 정화는 억눌린 열정과 인정욕의 상징이에요. '뜨겁게 인정받고 싶다, 내가 해낸 것을 누군가 정확히 알아봐 주었으면 좋겠다'는 욕구가 내면 깊숙이 있어요. 하지만 먼저 달라붙거나, 필요한 사람처럼 보이는 게 싫어서 절대 티를 안 내요. 그래서 진심을 모르는 사람은 무토(戊土)가 '욕심이 없는 사람'이라고 착각하기도 해요.",
      desire2: "토(土) 일간의 재성(財星) 구조: 재성은 내가 통제하고 움직이는 것이에요. 관계에서도 내가 먼저 가지러 가지 않아요. 상대가 먼저 다가오고, 먼저 원해야 해요. 그게 나의 방식이에요. 한번 마음을 열기로 결심하면 그것도 산처럼 확고하지만, 그 결심까지 오는 과정은 아무도 모르게 혼자 오래 검토해요.",
      triggers: [
      ...
      idealType2: "따뜻하고 감정적으로 성숙한 사람이 실제 궁합이에요. 나의 과묵함과 기복을 '차갑다'거나 '이상하다'고 느끼지 않고 그냥 담아줄 수 있는 넉넉함. 오행상 화(火) 일간 — 병화(丙火), 정화(丁火) — 이 찰떡 궁합이에요. 내가 부족한 화 기운을 상대가 채워주고, 나의 토(土)가 상대의 불을 받쳐주는 상생(相生) 구조예요. 병화는 나를 세상 밖으로 끌어내는 에너지, 정화는 내 내면의 뜨거움을 알아봐 주는 에너지예요."
    }
  },
  기: {
    ...
    recovery: "규칙적인 루틴 + 인정받는 경험 + 가까운 사람과의 깊은 대화: 화(火) 결핍을 채우는 가장 빠른 방법이에요. 요리, 원예, 산책처럼 무언가를 직접 돌보는 활동도 에너지를 회복시켜요.",
    day: {
    ...
    night: {
      ...
      desire2: "토(土) 일간의 재성 구조: 목(木)이 재성이에요. 기토(己土)는 목을 키워주는 대지예요. 관계에서도 상대가 성장하도록 돕고 지지하는 역할을 자연스럽게 맡아요. 나를 위해서가 아니라 상대를 위해서 무언가를 하는 것에서 보람을 느껴요.",
      triggers: [
      ...
      idealType2: "안정적이고 든든하게 방향을 잡아주는 사람이 실제 궁합이에요. 화(火) 일간 — 병화(丙火), 정화(丁火) — 이 잘 맞아요. 화생토(火生土), 불이 대지를 따뜻하게 만드는 에너지예요. 나의 습한 내면을 따뜻하게 데워주는 파트너예요."
    }
  },
  경: {
    ...
    night: {
      ...
      desire2: "금(金) 일간의 재성 구조: 목(木)이 재성이에요. 경금(庚金)은 갑목(甲木)을 베어서 형태를 만들어요. 관계에서도 상대를 내 기준으로 다듬고 변화시키려는 경향이 있어요. 그게 때로는 상대를 답답하게 만들어요.",
      triggers: [
      ...
      idealType2: "신뢰할 수 있고 약속을 지키는 사람이 실제 궁합이에요. 화(火) 일간 — 병화(丙火), 정화(丁火) — 이 잘 맞아요. 불로 쇠를 단련하는 화극금(火克金) 구조지만, 오히려 이 자극이 경금(庚金)을 빛나게 만들어요. 나를 편하게 놔두지 않는 도전적인 파트너가 맞아요."
    }
  },
  신: {
    ...
    night: {
      ...
      desire2: "금(金) 일간의 재성 구조: 목(木)이 재성이에요. 신금(辛金)의 관계 방식은 완벽하게 준비된 뒤에 나타나는 것이에요. 흠 없이 아름다운 모습으로 상대 앞에 서고 싶어하기 때문에, 준비가 안 됐다고 느끼면 관계를 미뤄요.",
      triggers: [
      ...
      idealType2: "안정적이고 나의 예민함을 감당해주는 사람이 실제 궁합이에요. 수(水) 일간 — 임수(壬水), 계수(癸水) — 이 잘 맞아요. 금생수(金生水), 신금(辛金)이 물을 만나 빛나는 구조예요. 나의 날카로운 면을 유연하게 흘려보내는 깊고 넓은 파트너가 맞아요."
    }
  },
  임: {
    ...
    night: {
      ...
      desire2: "수(水) 일간의 재성 구조: 화(火)가 재성이에요. 임수(壬水)의 관계 방식은 흐르듯 자연스럽게 다가가는 것이에요. 억지가 없고, 강요하지 않아요. 관계가 자연스럽게 깊어지는 것을 선호하고, 억지로 가까워지는 것을 본능적으로 경계해요.",
      triggers: [
      ...
      idealType2: "나의 복잡함을 이해해주는 사람이 실제 궁합이에요. 목(木) 일간 — 갑목(甲木), 을목(乙木) — 이 잘 맞아요. 수생목(水生木), 나의 깊은 에너지를 방향으로 만들어주는 구조예요. 함께 있어도 각자의 공간이 존재하면서 깊이 연결된 관계를 원해요."
    }
  },
  계: {
    ...
    night: {
      ...
      desire2: "수(水) 일간의 재성 구조: 화(火)가 재성이에요. 계수(癸水)의 관계 방식은 천천히, 조용히 스며드는 것이에요. 급하게 가까워지는 것보다 서서히 깊어지는 관계를 선호해요. 한번 마음이 열리면 완전히 열리는 구조예요.",
      triggers: [
      ...
      idealType2: "안정적이고 묵직하게 나를 받아주는 사람이 실제 궁합이에요. 토(土) 일간 — 무토(戊土), 기토(己土) — 이 잘 맞아요. 무계합(戊癸合)처럼 자연스럽게 끌리는 구조예요. 나의 이슬 같은 감성을 흡수하고 땅으로 만들어주는 안정적인 파트너예요."
    }
  },
};
...
function buildSajuData(input){
  ...
  const ilju_persona_desc = ILJU_CHAR[iljuKey] || `${OHK[ilO]} 기운의 일주예요.`;
  const ilgan_persona_desc = `${ilganDB[vsKey] || ""} ${ilganDB.core.replace(/^[^:：]*[：:]\s*/,"")}`.trim();
  const persona = [
  ...
  return{
    ...
    boundary:{...bnd,isBoundary:bnd.inBoundary,
      standardDesc:`${ilju.hanja}(${ilju.ko}): ${ILJU_CHAR[iljuKey]||OHK[ilO]+" 기운의 일주예요."}\n${ilganDB.core}`,
      midnightDesc:`${iljuB.hanja}(${iljuB.ko}): ${ILJU_CHAR[iljuB.ko]||OHK[normO(GAN_OE[ilganB])]+" 기운의 일주예요."}\n${(ILGAN_DESC[ilganB]||ILGAN_DESC["기"]).core}`,
    },
    pillars,pillarsB,
    ohaengDist,singang,
...
// 내면 해부 — 기본 6개 프롬프트
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function buildInnerPrompt(d){
  const dn=d.daynight;
  const ilgan=d.pillars?.[2]?.gan?.ko||"";
  const ilju=ilgan+(d.pillars?.[2]?.ji?.ko||"");
  const sinsal=(d.sinsal||[]).map(s=>s.name).join(", ")||"없음";
  const triggers=(dn?.night?.triggers||[]).join(" / ");
  return `다음은 ${d.name}님의 사주·심리 데이터야.
일주: ${ilju} / 일간: ${ilgan} / 신강신약: ${d.singang||""} / MBTI: ${d.mbti?.estimated||""} (${d.mbti?.userType||""})
용신: ${d.yongsinA||""} / 기신: ${d.gisinA||""}
감정 폭발 상황: ${triggers}
낮의 자아: ${dn?.day?.impression||""} / 밤의 욕구: ${dn?.night?.desire||""}
신살: ${sinsal} / 생명경로수: ${d.tarot?.lifePath||""}

위 데이터로 아래 6가지를 각각 분석해줘.
한자 절대 사용 금지. ~이에요/~해요 체. "${d.name}님"으로 지칭. 이모지·마크다운 금지.

1. angerTriggers (array 3개): 자존심/화를 건드리는 말 top3. 각각 {"quote":"말 자체","reason":"왜 이 말이 특히 찌르는지 2~3문장"}
2. innerRhythm (string): 분노 패턴과 회복 리듬 에세이. 문단1(150자): 감정이 올라오는 내면 패턴. 빈줄. 문단2(200자): 회복 방식.
3. exercise (array 3개): 개운 운동 추천. 각각 {"name":"운동명","reason":"왜 맞는지 2~3문장. 사주·별자리 근거 포함"}
4. hobby (array 3개): 개운 취미 추천. 각각 {"name":"취미명","reason":"왜 맞는지 2~3문장. 사주·별자리 근거 포함"}
5. moneyReason (string): 지금 돈이 없다면 왜인지. 재성·기신·별자리·생명경로수로. 300자 내외. 위로 톤.
6. jinsang (string): 이 사람이 진상일 때 패턴. 300자 내외. 자기객관화, 가볍고 공감되는 톤.
...
// 내면 해부 — 딥 카드 10개 프롬프트
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function buildDeepPrompt(key, d){
  const ilgan=d.pillars?.[2]?.gan?.ko||"";
  const ilju=ilgan+(d.pillars?.[2]?.ji?.ko||"");
  const base=`[입력 정보]
생년월일: ${d.birth} / 성별: ${d.gender} / 일주: ${ilju} / 일간: ${ilgan}
신강신약: ${d.singang||""} / MBTI: ${d.mbti?.estimated||""} / 생명경로수: ${d.tarot?.lifePath||""}
용신: ${d.yongsinA||""} / 기신: ${d.gisinA||""} / 신살: ${(d.sinsal||[]).map(s=>s.name).join(", ")||"없음"}
태양별자리: ${d._astroAI?.sun||d.astro?.sun||""} / 달별자리: ${d._astroAI?.moon||d.astro?.moon||""} / ASC: ${d._astroAI?.asc||d.astro?.asc||""}
당사주: ${(d.dansaju?.stars||[]).map(p=>p.byeolseong).join(", ")||""}
토정비결: ${d.tojung?.saja||""}`;
...
[출력 형식 — 반드시 준수]
- 말투: ~해요/~이에요 친근한 존댓말. "${d.name}님"으로 지칭. 한자·이모지·마크다운 금지.
- 구조: 문단1(3문장 이어쓰기) + 문단2(3문장 이어쓰기) + 결론(1문장 볼드 처리용 마커 없이 일반 텍스트)
- 계산 과정 없이 결론만. 쉬운 설명.

JSON만 응답 (다른 텍스트 없이):
{"para1":"...","para2":"...","conclusion":"결론 한 문장. 위로 문장 바로 이어서."}`;
}
...
function TabInner({d}){
  ...
  const [innerAI,setInnerAI]=React.useState(()=>{
    try{const s=localStorage.getItem(cacheKey("base"));return s?JSON.parse(s):null;}catch{return null;}
  });
  const [innerLoading,setInnerLoading]=React.useState(false);
  const [innerErr,setInnerErr]=React.useState(false);
  // 내면의 리듬 기본 열림
  const [rhythmOpen,setRhythmOpen]=React.useState(true);
  ...
  const [deepData,setDeepData]=React.useState(()=>{
    ...
    DEEP_CARDS.forEach(c=>{
      try{const s=localStorage.getItem(cacheKey(c.key));if(s)init[c.key]=JSON.parse(s);}catch{}
    });
    return init;
  });
  ...
  React.useEffect(()=>{
    ...
    async function run(){
      ...
      try{
        ...
        if(!cancelled){
          setInnerAI(parsed);
          try{localStorage.setItem(cacheKey("base"),JSON.stringify(parsed));}catch{}
        }
  ...
  async function fetchDeep(key){
    ...
    try{
      ...
      try{localStorage.setItem(cacheKey(key),JSON.stringify(parsed));}catch{}
  ...
  const SkelBlock=({err})=><div style={{paddingTop:4,display:"flex",flexDirection:"column",gap:4}}>
    {err?<p style={{fontSize:12,color:"#ef5350",margin:0}}>잠시 후 다시 시도해주세요.</p>
      :[100,90,95,80,85,70].map((w,i)=><Skel key={i} w={`${w}%`}/>)}
  </div>;

  // 아코디언 헤더 공통 컴포넌트
  const AccHead=({open,onToggle,icon,title,bgColor,borderColor,titleColor,dark})=>(
    <div onClick={onToggle} style={{
      display:"flex",alignItems:"center",justifyContent:"space-between",
      padding:"14px 16px",background:bgColor||"#fff",
      ...
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <span style={{fontSize:14,fontWeight:800,color:dark?"#e0e7ff":titleColor||"#111"}}>{title}</span>
      </div>
      <span style={{fontSize:12,color:dark?"#a78bfa":"#bbb",transform:open?"rotate(180deg)":"none",transition:"transform 0.2s"}}>▼</span>
    </div>
  );
  ...
  const DeepBody=({cardKey,bg,border,dark})=>{
    ...
    return(
      <div style={{padding:"14px 16px",background:dark?"#0f0e17":bg,border:`1px solid ${border}`,borderTop:"none",borderRadius:"0 0 12px 12px"}}>
        {(!data&&!loading&&!err)?<SkelBlock err={false}/>
        :loading?<SkelBlock err={false}/>
        :err?<SkelBlock err={true}/>
        :<>
          <p style={{fontSize:13,color:dark?"#c4b5fd":"#444",lineHeight:1.85,margin:"0 0 12px",textAlign:"justify"}}>{data.para1}</p>
          <p style={{fontSize:13,color:dark?"#c4b5fd":"#444",lineHeight:1.85,margin:"0 0 12px",textAlign:"justify"}}>{data.para2}</p>
  ...
  return <>
    ...
    {/* ── 내면의 리듬 (기본 열림, 병렬 로딩) ── */}
    <section style={S.card}>
      <div onClick={()=>setRhythmOpen(o=>!o)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",marginBottom:rhythmOpen?10:0}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:18}}>🌊</span>
          <div>
            <div style={{fontSize:14,fontWeight:800,color:"#111"}}>내면의 리듬</div>
            <div style={{fontSize:11,color:"#aaa"}}>분노 패턴 · 회복 리듬 · 감정 트리거</div>
          </div>
        </div>
        <span style={{fontSize:12,color:"#bbb",transform:rhythmOpen?"rotate(180deg)":"none",transition:"transform 0.2s"}}>▼</span>
      </div>
      {rhythmOpen&&(
        innerLoading?<SkelBlock err={false}/>
        :innerErr?<SkelBlock err={true}/>
        :innerAI?<>
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
            {(innerAI.angerTriggers||[]).map((t,i)=>(
              <div key={i} style={{padding:"12px 14px",background:"#fff5f5",borderRadius:11,border:"1px solid #ffcdd2"}}>
                <div style={{fontSize:13,fontWeight:900,color:"#b71c1c",marginBottom:6}}>"{t.quote}"</div>
                <p style={{fontSize:12,color:"#555",margin:0,lineHeight:1.75}}>{t.reason}</p>
              </div>
            ))}
          </div>
          <div style={{padding:"14px 16px",background:"linear-gradient(135deg,#1e1b4b,#312e81)",borderRadius:12}}>
            {(innerAI.innerRhythm||"").split("\n\n").map((para,i)=>(
              <p key={i} style={{fontSize:13,color:"#e0e7ff",lineHeight:1.9,margin:i>0?"12px 0 0":"0",textAlign:"justify"}}>{para}</p>
            ))}
          </div>
        </>:<SkelBlock err={false}/>
    ...
    {/* ── 기본 4개 아코디언 ── */}
    {[
      {key:"exercise",icon:"🏃",title:"운동 추천",render:()=>(
        <div style={{display:"flex",flexDirection:"column",gap:10,padding:"14px 16px",background:"#f0fdf4",border:"1px solid #bbf7d0",borderTop:"none",borderRadius:"0 0 12px 12px"}}>
          {(innerAI?.exercise||[]).map((ex,i)=>(
            <div key={i} style={{padding:"12px 14px",background:"#fff",borderRadius:10,border:"1px solid #bbf7d0"}}>
              <div style={{fontSize:13,fontWeight:900,color:"#166534",marginBottom:5}}>{ex.name}</div>
              <p style={{fontSize:12,color:"#444",margin:0,lineHeight:1.75}}>{ex.reason}</p>
            </div>
          ))}
        </div>
      )},
      {key:"hobby",icon:"🎨",title:"취미 추천",render:()=>(
        <div style={{display:"flex",flexDirection:"column",gap:10,padding:"14px 16px",background:"#fdf4ff",border:"1px solid #e9d5ff",borderTop:"none",borderRadius:"0 0 12px 12px"}}>
          {(innerAI?.hobby||[]).map((h,i)=>(
            <div key={i} style={{padding:"12px 14px",background:"#fff",borderRadius:10,border:"1px solid #e9d5ff"}}>
              <div style={{fontSize:13,fontWeight:900,color:"#6b21a8",marginBottom:5}}>{h.name}</div>
              <p style={{fontSize:12,color:"#444",margin:0,lineHeight:1.75}}>{h.reason}</p>
            </div>
          ))}
        </div>
      )},
      {key:"money",icon:"💸",title:"돈이 없는 이유",render:()=>(
        <div style={{padding:"14px 16px",background:"#fffbeb",border:"1px solid #fde68a",borderTop:"none",borderRadius:"0 0 12px 12px"}}>
          {(innerAI?.moneyReason||"").split("\n\n").map((p,i)=>(
            <p key={i} style={{fontSize:13,color:"#444",margin:i>0?"10px 0 0":"0",lineHeight:1.85,textAlign:"justify"}}>{p}</p>
          ))}
        </div>
      )},
      {key:"jinsang",icon:"😤",title:"내가 진상일 때는?",render:()=>(
        <div style={{padding:"14px 16px",background:"#fdf2f8",border:"1px solid #fbcfe8",borderTop:"none",borderRadius:"0 0 12px 12px"}}>
          {(innerAI?.jinsang||"").split("\n\n").map((p,i)=>(
            <p key={i} style={{fontSize:13,color:"#444",margin:i>0?"10px 0 0":"0",lineHeight:1.85,textAlign:"justify"}}>{p}</p>
          ))}
        </div>
      )},
      ...
      const bgMap={exercise:"#f0fdf4",hobby:"#fdf4ff",money:"#fffbeb",jinsang:"#fdf2f8"};
      const borderMap={exercise:"#bbf7d0",hobby:"#e9d5ff",money:"#fde68a",jinsang:"#fbcfe8"};
      const tcMap={exercise:"#166534",hobby:"#6b21a8",money:"#92400e",jinsang:"#831843"};
      return(
        <div key={key}>
          <AccHead open={open} onToggle={()=>{
            if(!innerAI&&!innerLoading&&!innerErr){/* 아직 로딩 전이면 무시 */return;}
            setBasicOpen(p=>({...p,[key]:!p[key]}));
          }} icon={icon} title={title} bgColor={bgMap[key]} borderColor={borderMap[key]} titleColor={tcMap[key]}/>
          {open&&(!innerAI?<SkelBlock err={innerErr}/>:render())}
        </div>
      );
    })}
    ...
    {DEEP_CARDS.map(card=>{
      ...
      return(
        <React.Fragment key={card.key}>
          {isNewGroup&&(
            <div style={{padding:"6px 4px 2px",fontSize:11,fontWeight:800,color:"#bbb",letterSpacing:0.5}}>
              {card.group}
            </div>
          )}
          <div>
            <AccHead open={open} onToggle={()=>toggleDeep(card.key)}
              ...
              bgColor={card.bg} borderColor={card.border} titleColor={card.tc} dark={card.dark}/>
...
function TabTojung({d}){
  ...
  return <>
    ...
    <section style={S.card}>
      ...
      <GT>토정비결은 조선 중기 이지함(李之菡) 선생이 집대성한 민간 예언서입니다. 사주를 수리화하여 한 해의 길흉과 본명 총운을 사자성어로 풀어냅니다.</GT>
...
function TabAstro({d}){
  ...
  async function fetchTarotAchieve(){
    ...
    try{
      ...
      const text=await callNetlify({model:"claude-haiku-4-5-20251001",max_tokens:400,messages:[{role:"user",content:prompt}]});
...
export default function SajuReport(){
  ...
  const TABS=["요약","사주","낮과 밤","내면 해부","토정·주역","별자리·타로","MBTI"];
    ...
    <div style={SF.root}>
      ...
      <div style={SF.content}>
        ...
        <div style={{padding:"12px 14px",background:"#e3f2fd",borderRadius:12,marginBottom:16,fontSize:12,color:"#0d47a1",lineHeight:1.7}}>
          ✨ 입력 정보로 사주 8자를 계산하고 종합 운세 리포트를 생성해요.
        </div>
