...
export default function MoraReport({ d, onHome, onSavePDF, pdfLoading, pdfMode, parentAstroAI, setParentAstroAI, parentTarotAI, setParentTarotAI }) {
 ...
 const personaHook = `${_persoCore} ${_punch}`
  ...
  const daeunCurText = curDaeun
    ? `지금은 ${_daeunLabel} 대운이야. ${_curDaeunTheme}의 시기라 타고난 재능을 펼쳐 보이기 좋은 흐름이야. ${daeunMeaning}`
    : "대운 읽는 중이야."
  ...
  const daeunNextText = nextDaeun
    ? `다음은 ${nextDaeun.label?.replace(/[一-龯\u4E00-\u9FFF（(][^）)]*[）)]/g, "").trim()} 대운이야. ${_nextStartYear ? `${_nextStartYear}년부터 흐름이 바뀌어. ` : ""}그 전에 지금 대운에서 쌓아둔 게 다음 십 년을 결정해. 전환점이 오기 전에 지금 이 시기를 알차게 써야 해. 미뤄둔 게 있으면 이 대운 안에 끝내라는 뜻이야.`
  ...
  const companyFitText2 = (() => {
    ...
    if (_foundYear) {
      ...
      ageTail = ` 참고로 ${_foundYear}년생 회사라 ${age < 5 ? "아직 어린 조직이야. 안정성은 덜해도 함께 자리 잡는 재미가 있어." : age < 15 ? "한창 성장하는 조직이라 체계와 기회가 함께 있어." : "충분히 자리 잡은 조직이라 안정적이지만 변화 속도는 느릴 수 있어."}`
    }
  ...
  const daeunTransitionText = (() => {
    ...
    const head = `${nextAge ? `만 ${nextAge}세 무렵` : "다음 대운"}에 대운이 ${next.label}로 바뀌어. 대운이 바뀌는 전후 2~3년은 인생의 환절기라, 겉으론 멀쩡해 보여도 속에서 판이 크게 흔들려. `
    const body = dir === "up"
