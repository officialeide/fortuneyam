...
const OHK_SEASON_WARN = {
  ...
  수: "물 기운이 약한데, 겨울에는 체력이 빨리 떨어져. 체온 유지랑 규칙적인 생활 리듬이 중요해.",
}
const OHK_FOOD = {
...
export default function MoraReport({ d, onHome, onSavePDF, pdfLoading, pdfMode, parentAstroAI, setParentAstroAI, parentTarotAI, setParentTarotAI }) {
  ...
  const fiveViewText = `사주는 ${dominant ? OHK_KR[dominant] : ""} 기운의 사람이라 말하고, 당사주와 토정비결도 비슷한 결을 짚어. 서양 별자리는 태양 ${sunSign || "별자리"}로 같은 기질을 다른 언어로 그려내고, 타로 생명경로수는 ${tarotCardName} 카드로 그 삶의 방향을 다시 확인해. 동양은 기운으로, 서양은 별과 상징으로 읽었지만, 다섯 관점이 가리키는 건 결국 한 사람이야. 서로 다른 지도가 같은 목적지를 가리킬 때, 그게 진짜라는 증거야. ${_actionByOh[dominant] || _actionByOh["토"]}`
  ...
  const fiveViewTensionText = (() => {
    ...
    let body
    if (!elem) {
      body = `이 사주는 ${sajuLabel} 기운이 중심이야. 태어난 시간이 없어 별자리까지 겹쳐 볼 수는 없지만, 동양의 결 안에서도 밀고 당기는 두 힘을 함께 봐. 안정을 원하면서 동시에 더 뻗고 싶은 마음, 그 진폭이 이 사람의 입체감이야.`
    } else if (clash) {
      body = `사주는 ${sajuLabel}을 말하는데, 별자리는 ${astroLabel} 기질을 가리켜. 방향이 정반대로 엇갈리지만 결함이 아니야. 한쪽으로만 쏠린 사람은 납작한데, 이 사주는 두 힘이 팽팽하게 당겨 입체가 생겨. ${tarotDrive ? `타로 생명경로수는 ${tarotDrive}의 결이라 또 다른 각도를 얹어. ` : ""}겉은 안정을 지키면서 속으로 딴 세상을 꿈꾸는 그 낙차가 매력이자 숙제야. 이 간극을 억지로 누르지 말고, 두 힘을 다 쓰는 법을 익히는 게 과제야.`
    } else {
      body = `사주도 별자리도 ${sajuLabel} 쪽으로 결이 비슷하게 모여. 드물게 안팎이 한 방향인 구조라 흔들려도 결국 제자리를 찾아와. ${tarotDrive ? `타로 생명경로수의 ${tarotDrive} 기운만 살짝 다른 각을 주는데, ` : ""}그 작은 어긋남이 지루함을 깨는 변주야. 방향이 뚜렷한 만큼 큰 방향을 믿고 가되, 가끔은 딴길의 신호도 무시하지 마.`
    }
    return body
  })()
  ...
  const _punch = {
    비겁: "근데 그거 알아? 무리에 섞여 있다가도 끝내 자기 두 발로 서고 마는 사람이야. 남한테 기대는 것 같아도 결정적인 순간엔 결국 혼자 판을 뒤집어. 그 꺾이지 않는 심지가 이 사주의 진짜 척추야.",
    식상: "근데 그거 알아? 머릿속에선 이미 남보다 열 걸음 앞서 그림을 그리고 있어. 다들 '그게 되겠어?' 할 때 혼자 답을 보고 있지. 그 상상력이 결국 재능으로 증명되는 사람이야.",
    재성: "근데 그거 알아? 원하는 건 기어이 손에 쥐고야 마는 현실 감각이 있어. 순하고 무던해 보여도 필요할 땐 누구보다 계산이 빠르고 독해. 그 야무짐이 결국 이 사람을 남다르게 만들어.",
    관성: "근데 그거 알아? 무너질 것 같은 순간에 오히려 더 반듯해지는 사람이야. 다들 흔들릴 때 혼자 중심을 잡아서, 그 무게가 곁의 사람들까지 붙잡아 줘. 그 책임감이 이 사주의 진짜 그릇이야.",
    인성: "근데 그거 알아? 혼자 삭이는 그 깊이가 결국 남들이 못 보는 걸 먼저 읽어내는 눈이 돼. 조용히 담아둔 만큼 통찰이 깊어서, 결정적일 때 판을 읽는 힘으로 터져. 그게 이 사주의 진짜 무기야.",
  }[_topSibsong] || "근데 그거 알아? 남들이 못 보는 자기만의 결을 끝내 찾아내고야 마는 사람이야. 그 고집스러운 촉이 결국 이 사람을 남다르게 만들어."
 ...
 const personaHook = `${_persoCore} ${_punch}`
  ...
  const wealthPatternText = pick5({
    비겁: "능력은 있어. 근데 혼자 다 하려다 힘이 분산돼. 사람과 나누고 판을 키울 때 오히려 돈이 붙는데, 동업이나 돈거래에선 뺏기기 쉬우니 선은 분명히 그어.",
    식상: "능력은 있어. 근데 만들기만 하고 파는 걸 놓쳐. 아이디어를 돈으로 바꾸는 마지막 한 걸음을 남에게 미루면 제자리야. 표현하고 내다 파는 데까지 가야 돈이 돼.",
    재성: "능력은 있어. 돈 감각도 좋아. 근데 여러 군데 벌여서 다 얕게 먹어. 하나에 집중해 깊게 팔 때 비로소 큰돈이 붙어.",
    관성: "능력은 있어. 인정도 받아. 근데 자리나 직책이 서야 돈이 따라오는 구조야. 눈앞의 벌이보다 나를 인정해줄 판에 올라타는 게 먼저야.",
    인성: "능력은 있어. 근데 배우고 준비만 하다 실전 타이밍을 놓쳐. 전문성이 돈으로 연결되려면, 완벽히 준비되기 전에 한 발 먼저 내미는 용기가 필요해.",
  }) + " 지금까지 에너지가 살아났던 일과 지쳤던 일을 돌아봐. 살아난 쪽이 용신 방향이야."
  ...
  const daeunMeaning = curDaeun
    ? (_yongList.includes(_curDaeunO)
        ? `이 대운은 용신인 ${OHK_KR[_curDaeunO]||_curDaeunO} 기운이 들어오는 구간이야. 십 년 중 가장 크게 치고 나갈 수 있는 시기라, 미뤄뒀던 일을 벌이고 판을 키워야 해. 이때 쌓은 게 평생 가.`
        : _giList.includes(_curDaeunO)
        ? `이 대운은 기신인 ${OHK_KR[_curDaeunO]||_curDaeunO} 기운이 강해지는 구간이야. 무리하게 확장하면 힘만 빠져. 새 판을 벌이기보다 실력을 다지고 다음 대운을 준비하는 게 이 십 년을 잘 쓰는 법이야.`
        : `이 대운의 ${OHK_KR[_curDaeunO]||_curDaeunO} 기운은 용신도 기신도 아닌 중립 구간이야. 크게 흔들리지 않는 대신 저절로 풀리지도 않아. 스스로 방향을 정하고 꾸준히 밀고 가는 사람이 결과를 만들어.`)
    : ""
  ...
  const daeunCurText = curDaeun
    ? `지금은 ${_daeunLabel} 대운이야${_curRange ? `(${_curRange})` : ""}. ${_curDaeunTheme}의 시기라 타고난 재능을 펼쳐 보이기 좋은 흐름이야. ${daeunMeaning}`
    : "대운 읽는 중이야."
  ...
  const daeunNextText = nextDaeun
    ? `다음은 ${nextDaeun.label?.replace(/[一-龯\u4E00-\u9FFF（(][^）)]*[）)]/g, "").trim()} 대운이야${_nextRange ? `(${_nextRange})` : ""}. ${_nextStartYear ? `${_nextStartYear}년부터 흐름이 바뀌어. ` : ""}그 전에 지금 대운에서 쌓아둔 게 다음 십 년을 결정해. 전환점이 오기 전에 지금 이 시기를 알차게 써야 해. 미뤄둔 게 있으면 이 대운 안에 끝내라는 뜻이야.`
  ...
  const childhoodText = (() => {
    ...
    let base
    if (["편인","정인"].includes(yj)) base = "유년기에 보살핌과 관심을 넉넉히 받고 자란 편이야. 그 안정감이 지금의 정서적 뿌리가 됐는데, 기대와 간섭도 함께 컸을 수 있어."
    else if (["편관","정관"].includes(yj)) base = "유년기에 규율과 기대 속에서 자란 편이야. 엄한 분위기가 일찍 철들게 만든 만큼, 마음껏 응석 부릴 틈은 적었을 수 있어."
    else if (["편재","정재"].includes(yj)) base = "유년기에 현실적이고 활기찬 가정에서 자란 편이야. 부모가 바쁘게 살림을 꾸리는 걸 보며 생활력과 눈치를 일찍 익혔어."
    else if (["식신","상관"].includes(yj)) base = "유년기에 자유롭고 표현이 허용되는 분위기에서 자란 편이야. 그게 지금의 창의성과 표현력으로 이어졌어."
    else base = "유년기의 가정 분위기가 지금의 정서적 뿌리를 만들었어. 그때 부모와 맺은 방식이 지금 곁을 내주는 방식에 남아 있어."
    // 아버지(편재) — 존재감·거리, 0개/1개/2개 이상 3단계
    const pj = d.sibsongAnalysis?.counts?.["편재"] || 0
    const father = pj >= 2
      ? " 아버지의 존재감이 유독 뚜렷했을 사주야. 밖에서 부지런히 생활을 책임진 분이었을 가능성이 크고, 그 무게가 집안 전체 분위기를 좌우했을 수 있어."
      : pj === 1
      ? " 아버지는 밖에서 부지런히 생활을 책임진 분이었을 가능성이 커. 든든했지만 정서적으로는 조금 먼 느낌이 남았을 수 있어."
    ...
    const mother = mj >= 2
      ? " 어머니의 영향이 특히 강했던 사주야. 곁에서 세심하게 챙겨준 만큼, 그 관심이 간섭처럼 느껴지는 순간도 잦았을 수 있어."
      : mj === 1
      ? " 어머니는 곁에서 감싸주는 편이었지만, 그 관심이 때로 간섭처럼 느껴졌을 수도 있어."
      : " 어머니의 손길은 옅은 편이라 정서적으로 일찍 자립하는 법을 배웠어."
    // 오행 결핍 → 자란 환경 추론
    const env = missingOh.length
      ? ` 오행에 ${missingOh.map(k => OHK_KR[k]).join(", ")} 기운이 비어, 그 부분은 스스로 채우게 만든 환경이었어.`
      : ""
    return base + father + mother + env
  })()
  const _JI_OH_LOOK = { 자:"수", 축:"토", 인:"목", 묘:"목", 진:"토", 사:"화", 오:"화", 미:"토", 신:"금", 유:"금", 술:"토", 해:"수" }
  const _SPOUSE_LOOK = {
    목: "외모로는 키가 크고 선이 곧은 편이 많아. 늘씬하고 이목구비가 시원시원하며, 어딘가 젊고 반듯한 인상을 주는 사람이야.",
    화: "외모로는 화사하고 눈에 띄는 인상이 많아. 눈빛이 또렷하고 표정이 풍부하며, 함께 있으면 분위기가 밝아지는 사람이야.",
    토: "외모로는 둥글고 편안한 인상이 많아. 후덕하고 안정감 있는 얼굴에, 처음 봐도 사람을 마음 놓이게 하는 온화한 분위기야.",
    금: "외모로는 피부가 희고 깔끔한 인상이 많아. 골격이 분명하고 이목구비가 단정하며, 차분하면서도 야무진 느낌을 주는 사람이야.",
    수: "외모로는 부드럽고 촉촉한 인상이 많아. 눈이 크고 이목구비가 둥글며, 은근한 매력이 있어 오래 봐도 질리지 않는 사람이야.",
  }
  const spousePalaceText = (() => {
    ...
    let core
    if (["편재","정재"].includes(ilji)) core = "배우자 자리에 재성이 앉아 있어. 현실적이고 생활력 있는 짝을 만날 가능성이 커. 알뜰하게 살림을 꾸리는 상대라 결혼 후 경제적으로 안정되는 흐름이야. 다정하고 사교적이지만, 현실 감각이 뚜렷한 만큼 가끔 계산이 빠르다고 느껴질 수도 있어."
    else if (["편관","정관"].includes(ilji)) core = "배우자 자리에 관성이 앉아 있어. 듬직하고 책임감 있는 짝, 반듯한 상대를 만날 가능성이 커. 원칙이 분명해 나를 세워주는 사람이라 삶의 기둥이 되어주지만, 자기 기준이 확고해서 때로 엄격하게 느껴질 수 있어."
    else if (["편인","정인"].includes(ilji)) core = "배우자 자리에 인성이 앉아 있어. 따뜻하고 나를 챙겨주는 짝, 정신적으로 기댈 수 있는 상대를 만날 가능성이 커. 힘들 때 마음의 안식처가 되어주지만, 표현이 은근해서 애정을 눈에 보이게 확인하고 싶은 사람에겐 답답할 수도 있어."
    else if (["식신","상관"].includes(ilji)) core = "배우자 자리에 식상이 앉아 있어. 표현이 풍부하고 함께 있으면 즐거운 짝을 만날 가능성이 커. 재치 있고 감각적이지만, 자유로운 기질이 강해 얽매이는 걸 싫어하니 서로의 공간을 존중해야 오래가."
    else core = "배우자 자리에 비겁이 앉아 있어. 나와 비슷한 기질의 짝, 친구처럼 편안한 상대를 만날 가능성이 커. 대등하게 손잡고 가는 동반자형이지만, 둘 다 자기 색이 강하면 주도권을 두고 부딪힐 수 있으니 서로의 영역을 인정해줘."
  ...
  const soloApproachText = ({
    ...
    물: "천천히 스며들 듯 다가가. 몰아붙이면 도망가니, 편안한 대화와 여백을 주면서 자연스럽게 거리를 좁히는 게 유일하게 통해.",
  ...
  const reomulType = _jaeStruct === "편재"
    ...
    : _jaeStruct === "정재"
    ? `돈이 또박또박 들어오는 구조야. 한 방보다 꾸준히 쌓을 때 빛나는 사주라 월급이나 고정수입 같은 안정적인 흐름이 잘 맞아. 조급하게 큰 걸 노리다 페이스를 잃으니, 복리처럼 천천히 불리는 게 정답이야.`
    : _jaeStruct === "과다"
  ...
  const reomulInvest = _jaeStruct === "편재"
    ...
    : _jaeStruct === "정재"
    ? "차곡차곡 적립하고 복리로 굴리는 안정형이 맞아. 한 번에 크게 노리다 페이스 잃으니, 지루해도 꾸준한 게 정답이야. 남들이 단타로 크게 버는 걸 봐도 흔들리지 말고, 시간을 내 편으로 만드는 장기 투자가 결국 이 사주를 부자로 만들어."
    : _jaeStruct === "과다"
  ...
  const assetClassJSX = React.createElement("div", null,
    ...Object.entries(assetClass).map(([k, v], i, arr) =>
      React.createElement("div", { key: k, style: { marginBottom: i < arr.length - 1 ? 14 : 0 } },
  ...
  const ATTRACTIVE_BY_OH = {
    목: "말 안 해도 느껴지는 추진력에 상대가 끌려. 방향을 잡고 나아가는 모습 자체가 매력이야. 우유부단한 사람들 사이에서 특히 눈에 띄어. 뭘 하든 망설임 없이 밀고 나가는 태도가, 곁에 있는 사람한테는 든든함으로 다가가. 같이 있으면 나까지 뭔가 해낼 수 있을 것 같은 에너지를 줘서, 상대는 자기도 모르게 자꾸 곁을 맴돌게 돼. 정작 본인은 그게 매력인 줄 잘 모르는 경우가 많은데, 이 추진력이야말로 가장 먼저 상대 마음을 흔드는 지점이야.",
    화: "존재감과 에너지 자체가 매력이야. 같이 있으면 분위기가 사는 느낌을 줘서, 상대는 자기도 모르게 자꾸 곁에 있고 싶어 해. 표정이나 리액션이 살아있어서, 대화할 때 상대가 자기 얘기를 더 하고 싶어지게 만들어. 밝은 에너지가 전염되듯 퍼져서, 옆에 있는 사람까지 기분이 좋아지는 걸 스스로도 느끼게 돼. 화려하게 꾸미지 않아도 존재 자체로 시선을 끄는 타입이라, 상대는 이 사람 앞에서 자꾸 더 좋은 모습을 보이고 싶어져.",
    토: "흔들리지 않는 안정감에 끌려. 오래 봐도 편안한 사람이라는 인상을 줘서, 상대는 이 사람 앞에서만큼은 긴장을 풀게 돼. 감정 기복 없이 한결같은 태도가, 여러 사람을 만나본 상대일수록 오히려 더 크게 다가가는 매력이야. 급하게 다가오지 않고 묵묵히 곁을 지키는 모습에서, 상대는 이 사람이라면 믿고 기댈 수 있겠다는 확신을 갖게 돼. 화려하진 않아도 시간이 지날수록 이 편안함을 놓치기 싫어지는, 은근하지만 강한 끌림이야.",
    금: "분명한 기준과 태도에서 신뢰를 느껴. 흐리지 않고 확실한 게 매력이라, 상대는 이 사람 말은 믿을 수 있다는 확신을 갖게 돼. 우유부단하게 흐르지 않고 자기 생각을 정확히 말하는 모습이, 상대 입장에선 오히려 신선하고 매력적으로 느껴져. 겉으로는 조금 차가워 보여도, 한번 마음을 준 상대한테는 확실하게 잘해주는 반전 매력도 있어. 애매한 관계를 못 견디는 성격이라, 상대는 이 사람과 있으면 관계가 명확해진다는 안정감을 느끼게 돼.",
    수: "잔잔한 분위기 속 깊이에 끌려. 알아갈수록 매력이 드러나는 타입이라, 상대는 처음보다 시간이 지날수록 더 빠져들어. 말수는 적어도 눈빛이나 태도에서 깊은 생각이 느껴져서, 상대는 이 사람 속을 더 알고 싶다는 호기심을 갖게 돼. 감정을 함부로 드러내지 않는 만큼, 아주 가끔 보여주는 진심이 훨씬 크게 다가가는 구조야. 처음엔 잘 안 잡힐 것 같던 사람이 어느 순간 마음을 열어줄 때, 그 낙차가 상대를 완전히 사로잡아.",
  }
  ...
  const parentSupportText = (() => {
    const money = _jaeCnt >= 2
      ? "재성이 두둑한 사주라 부모나 집안의 물질적 뒷받침을 받을 구조야. 시작점에서 밀어주는 힘이 있어 남들보다 덜 헤매고 출발해."
      : _jaeCnt === 0
      ? "재성이 약해서 부모의 물질적 도움은 크게 기대하기 어려운 구조야. 일찍부터 내 힘으로 서는 자수성가형이야."
      : "재성이 적당히 있어서 부모의 도움이 아주 없진 않지만, 결정적인 고비에선 결국 내 힘으로 서야 해."
    const mind = _inCnt >= 2
      ? "정서적으로는 인성이 두터워 기댈 언덕이 있는 집안이야. 어머니 같은 존재의 보살핌이 깊어 힘들 때 돌아갈 뿌리가 있어."
      : _inCnt === 0
      ? "정서적으로는 인성이 약해 정신적 지지를 스스로 채워야 하는 구조야. 부모의 표현 방식이 내가 바라던 결과 달라, 일찍 마음을 다독이는 법을 익혔어."
      : "정서적으로는 인성이 적당해 지지가 아주 없진 않지만, 늘 채워지진 않아 스스로 다독이는 힘도 길렀어."
    return money + " " + mind
  })()
  const reomulHabitText = _jaeCnt >= 3
    ? "재성이 넘치는 사주라 돈 들어오는 길이 여러 개인데, 그만큼 새는 구멍도 많아. 벌이가 좋아도 통장이 안 불어나면 십중팔구 관리 문제야. 계좌를 목적별로 쪼개고, 자동으로 저축부터 떼어놓는 시스템을 만들어. 버는 재주는 확실하니 관리만 잡히면 재물이 가장 빠르게 불어나는 구조야."
    : _jaeCnt === 2
    ...
    : _jaeCnt === 1
    ? "재성이 하나 있는 사주라 큰돈이 몰리기보다, 꾸준한 흐름으로 쌓이는 구조야. 무리하게 확장하기보다 있는 걸 지키면서 조금씩 늘리는 방식이 잘 맞아. 정기적인 저축과 소비 기록만 붙여도 흐름이 눈에 보이기 시작해."
  ...
  const coupleSelfText = (isSingang
    ...
    편재: " 재물 씀씀이가 시원시원한 편이라, 데이트나 이벤트에 아낌없이 쓰는 스타일이기도 해.",
    정재: " 관계에서도 알뜰하고 계획적인 편이라, 미래를 같이 그리는 대화를 좋아해.",
    과다: " 이것저것 챙길 게 많아서 마음은 넉넉해도 정작 자기 자신은 뒷전이 되기 쉬워.",
    없음: " 물질적인 표현보다 마음과 시간을 내주는 쪽으로 애정을 표현하는 편이야.",
    균형: " 상황에 맞게 유연하게 맞춰가는 편이라 큰 갈등 없이 조율하는 힘이 있어.",
  }[_jaeStruct] || "")
  ...
  const coupleDeepenText = (isSingang
    ...
    목: ` 특히 ${yongsinD} 기운이 필요한 사주라, 둘이 같이 새로운 걸 시작하거나 계획을 세울 때 관계가 한층 살아나.`,
    화: ` 특히 ${yongsinD} 기운이 필요한 사주라, 감정과 애정을 확실하게 표현해줄 때 관계가 한층 살아나.`,
    토: ` 특히 ${yongsinD} 기운이 필요한 사주라, 안정적이고 예측 가능한 루틴을 함께 만들어갈 때 관계가 한층 단단해져.`,
    금: ` 특히 ${yongsinD} 기운이 필요한 사주라, 서로 간의 약속과 원칙을 분명히 지킬 때 관계가 한층 단단해져.`,
    수: ` 특히 ${yongsinD} 기운이 필요한 사주라, 말하지 않아도 이해받는다는 느낌을 받을 때 관계가 한층 깊어져.`,
  }[yongsinA?.split("·")[0]] || "") : "")
  // 커플 전용 결혼·동거 타이밍
  const _bestYearGap = bestYear ? bestYear.year - new Date().getFullYear() : null
  const marriageTimingText = `결혼이나 동거 같은 큰 결정은 억지로 날을 잡는다고 되는 게 아니라, 대운과 세운이 맞물릴 때 자연스럽게 무르익어. ${
    _bestYearGap !== null && _bestYearGap <= 2
      ? `세운으로 보면 바로 ${bestYear.year}년 전후가 관계를 한 단계 진전시키기 좋은 흐름이야. 멀지 않은 시기니 미리 마음의 준비를 해둬.`
      : _bestYearGap !== null
      ? `세운으로 보면 ${bestYear.year}년 전후가 관계를 한 단계 진전시키기 좋은 흐름이야. 아직 시간이 있으니 그 전까지 관계를 단단히 다져가면 돼.`
      : "향후 몇 년 안에 관계를 정리하고 다음 단계로 넘어갈 결정적인 시기가 와."
  } 둘 사이 신뢰가 충분히 쌓였을 때, 아래 애정 흐름에서 점수가 높은 달을 골라 중요한 이야기를 꺼내면 성공률이 훨씬 높아. 반대로 서로 지쳐 있거나 흐름이 가라앉은 달에 무리하게 밀어붙이면, 될 일도 어긋나기 쉬우니 때를 보는 게 중요해.`
  ...
  const slumpText = `권태기는 애정이 사라져서가 아니라, 익숙함에 서로를 새로 안 보게 될 때 찾아와. ${isSingang ? "이끄는 쪽이라 관계를 혼자 짊어지다 지칠 때 특히 위험한데, 상대에게 리드할 자리를 내주면 다시 생기가 돌아." : "맞춰주다 내 마음을 자꾸 미룰 때 애정이 식는데, 참기보다 바라는 걸 솔직히 말하면 오히려 관계가 살아나."} ${pick5({
    관성: "책임감에 짓눌려 관계마저 '해내야 할 일'처럼 느껴질 때가 위험 신호야. 가끔은 잘하려 하지 말고 그냥 같이 있는 시간을 즐겨.",
    인성: "머리로만 관계를 이해하려다 정작 마음이 따라가지 못할 때가 위험 신호야. 분석보다 그냥 느끼는 시간을 늘려봐.",
    식상: "표현이 습관처럼 반복되면 진심이 옅어 보일 수 있어. 가끔은 예상 못한 방식으로 마음을 꺼내봐.",
    비겁: "혼자만의 공간을 지키려다 상대를 자꾸 밀어내는 게 위험 신호야. 곁을 조금 더 내주는 연습이 필요해.",
    재성: "각자 챙기기 바빠 관계가 뒷전으로 밀릴 때가 위험 신호야. 둘만의 시간을 의식적으로 떼어놔야 해.",
  })} 같이 새로운 걸 경험하거나 당연하게 여겼던 걸 다시 고마워하는 것만으로도 정체된 흐름이 풀려. 권태기를 위기로만 보지 말고 관계가 한 단계 깊어지는 통과의례로 여기면, 이 고비를 넘긴 뒤 오히려 더 단단해져.`
  // 커리어 상세
  ...
  const yeonbongText = _jaeCnt >= 2
    ...
    : _jaeCnt === 1
    ? `재성이 하나 있어. 연봉은 무리하게 밀어붙이기보다 근거를 쌓아서 협상하는 쪽이 잘 맞아. 성과를 미리 정리해두면 협상 테이블에서 훨씬 유리해져.`
    : `재성이 없어. 돈 얘기를 꺼내는 것 자체가 어색하게 느껴질 수 있어. 하지만 말하지 않으면 아무도 안 챙겨주는 구조니, 데이터와 성과로 근거를 만들어서 담담하게 요구하는 연습이 필요해.`
  ...
  const burnoutText = `${isSingang ? "혼자 다 짊어지다 어느 순간 방전되는 게 번아웃 신호야. 다 잘하려 말고 맡길 건 맡겨." : "거절 못 하고 떠안다가 속으로 곪는 게 슬럼프의 시작이야. 내 한계를 인정하고 선을 긋는 연습이 필요해."} ${gisinA ? `특히 ${gisinD} 기운이 강해지는 시기엔 유독 지치고 판단이 흐려지니, 이때는 새 일을 벌이기보다 몸과 마음을 정비해.` : "무리가 겹치는 시기엔 성과보다 회복을 우선해."} 쉬는 것도 실력이야. 방전되기 전에 미리 숨 고르는 게 오래 일하는 비결이야.`
  // 이직운 (관성 + 역마 + 대운)
  const transitionPrepText = pick5({
    ...
    재성: "대운 전환기엔 벌인 판을 슬림하게 정리하고 다음 기회를 노려. 흐름을 읽고 미리 방향을 틀어두면 전환기가 오히려 크게 버는 발판이 돼. 지금 정리하는 손해가 다음 대운에서는 더 큰 이득으로 돌아올 거야.",
  })
  ...
  const chwiupText = `${hasJoin ? "지금 회사와 별개로 취업과 이직운을 보면," : "아직 다니는 회사가 없거나 입사일을 안 넣었으니 취업운으로 봐줄게."} ${_gwanCnt >= 1 ? "관성이 있어서 조직에 붙는 힘은 있어. 자기소개서나 면접에서 '책임감 있고 신뢰 가는 사람'으로 밀면 먹혀." : "관성이 약해서 대형 조직에 정공법으로 붙기보다, 전문성이나 실무 능력을 앞세우는 게 유리해. 작은 조직이나 실력 중심의 자리가 더 잘 맞아."} 합격운은 인성이 좌우하는데, ${_inCnt >= 1 ? "인성이 받쳐줘서 준비한 만큼 결실이 나와. 시험, 서류 운이 나쁘지 않아." : "인성이 약하니 붙을 때까지 두드리는 끈기가 필요해."} 아래 12개월 흐름에서 점수 높은 달에 원서를 넣어.`
  ...
  const HEALTH_MIND_BY_OH = {
    목: "간과 근육 쪽이 예민한 사주라, 스트레스를 받으면 마음보다 몸이 먼저 뻐근하게 반응해. 억지로 참지 말고 몸을 움직여서 푸는 게 이 사주엔 가장 빠른 회복법이야. 스트레칭이나 가벼운 운동으로 막힌 기운을 흘려보내는 습관을 들여. 몸이 풀리면 마음도 자연스럽게 따라 풀리는 구조야.",
    화: "심장과 혈이 예민한 사주라, 감정이 확 올라오면 몸이 바로 반응하는 타입이야. 흥분하거나 열받는 순간을 빨리 가라앉히는 게 건강의 핵심이야. 화가 나거나 들뜬 감정을 오래 붙들지 말고, 그때그때 식히는 루틴이 필요해. 마음의 온도를 잘 조절하는 게 이 사주의 평생 숙제야.",
    토: "위장과 소화기가 예민한 사주라, 스트레스가 제일 먼저 배로 내려가는 타입이야. 규칙적인 식사와 마음의 안정이 몸 상태를 그대로 좌우해. 걱정이 많아지면 소화부터 무너지니, 생각을 너무 오래 붙들고 있지 않는 연습이 필요해. 몸을 편하게 두는 것만으로도 마음이 같이 안정돼.",
    금: "폐와 호흡기가 예민한 사주라, 마음이 답답할 때 숨부터 얕아지는 타입이야. 감정을 억누르지 말고 제때 풀어주는 게 몸을 지키는 길이야. 화를 삭이기만 하면 가슴이 답답해지니, 말로든 글로든 밖으로 꺼내는 습관이 필요해. 숨을 깊게 쉬는 시간을 자주 만드는 것만으로도 몸과 마음이 함께 편해져.",
    수: "몸보다 마음이 먼저 지치는 사주야. 스트레스가 몸으로 내려오기 전에 마음을 비우는 루틴을 만들어. 명상이든 산책이든 혼자만의 회복 시간이 약보다 중요해. 마음이 편해야 몸도 따라서 편해지는 구조라, 감정을 방치하지 않는 게 가장 확실한 건강 관리야.",
  }
  ...
  const daeunGoldenText = _peakDaeun
    ? `인생 전체 대운을 펼쳐 보면, ${_peakLabelClean} 대운${_peakRange ? `(${_peakRange})` : ""}이 가장 크게 열리는 황금기야. 이 구간에 인생의 승부를 걸어. 여기서 벌인 일이 평생을 먹여 살려. 지금이 그 전이라면 이 시기를 위해 실탄을 모으고, 이미 지났다면 그때 쌓은 걸 어떻게 굴릴지가 관건이야.`
    : ""
  ...
  const lifeEarlyText = _phaseEarly ? `초년기는 ${_phaseEarly} 기운 속에 있어. ${_phaseEarly.includes("버티") ? "일찍부터 크고 작은 고생을 겪으며 뿌리를 내리는 시기라 마냥 편하진 않았을 거야. 하지만 이때 몸으로 부딪혀 배운 게 평생 밑천이 돼." : "받쳐주는 기운 속에서 비교적 사랑과 관심을 받고 컸을 가능성이 커. 이때 쌓인 안정감이 평생 정서의 바탕이 돼."} 부모나 집안의 영향을 강하게 받는 구간이라, 그 시절의 결핍이든 사랑이든 그게 지금의 나를 만든 원형이라는 걸 기억해.` : ""
  const lifeYoungText = _phaseYoung ? `청년기는 ${_phaseYoung} 기운 속에 있어. 세상에 나가 자기 자리를 만들어가는 가장 치열한 구간이라, ${_phaseYoung.includes("크게 열려") ? "치고 나갈 힘이 제대로 실려. 겁내지 말고 판을 벌여도 되는 시기야. 이때 도전한 게 인생 후반의 발판이 돼." : _phaseYoung.includes("버티") ? "당장 결과가 안 나와도 조급해하지 마. 실력과 사람을 쌓아두면 이때 버틴 힘이 나중에 한꺼번에 터져." : "요란하진 않아도 꾸준히 밀면 착실하게 기반이 잡혀. 큰 승부보다 하나씩 쌓는 게 정답이야."} 일이든 사랑이든 돈이든 인생의 큰 뼈대가 대부분 이 시기에 잡히니, 남과 비교하지 말고 내 속도를 지키는 게 관건이야.` : ""
  const lifeMidText = _phaseMid ? `중년기는 ${_phaseMid} 기운 속에 있어. 그동안 쌓아온 걸 실제로 거둬들이는 결실의 구간이라, ${_phaseMid.includes("크게 열려") ? "인생의 정점이 바로 여기 있어. 능력도 인맥도 무르익어 마음먹은 걸 가장 크게 펼칠 수 있으니, 이 황금기를 놓치지 마." : _phaseMid.includes("버티") ? "무리하게 판을 키우기보다 지킬 걸 단단히 지키는 게 이득이야. 내실을 다지며 다음 흐름을 기다려." : "요란하진 않아도 쌓아온 만큼 안정적으로 성과가 돌아와. 꾸준함이 신뢰로 바뀌는 시기야."} 책임이 가장 무거워지는 때라, 성취만큼 건강과 관계를 함께 챙기는 게 오래 가는 비결이야.` : ""
  const lifeLateText = _phaseLate ? `말년기는 ${_phaseLate} 기운 속에 있어. ${_phaseLate.includes("크게 열려") ? "늦게 피는 꽃이라 후반이 오히려 화려한 사주야. 젊을 때보다 지금이 더 편하고 풍요로울 수 있어, 그동안의 고생을 보상받는 시기야." : _phaseLate.includes("버티") ? "욕심을 내려놓고 지금 가진 걸 지키는 데 집중할 시기야. 몸과 마음의 평안을 우선하면 조용하지만 단단한 후반을 보낼 수 있어." : "안정 속에서 결실을 천천히 누리는 때야. 서두를 것 없이 쌓아온 것들이 주는 여유를 즐기면 돼."} 물질적 성취보다 마음의 평화와 사람과의 정이 더 큰 자산이 되는데, 이 평안은 앞선 세 구간을 어떻게 살았느냐가 결정해.` : ""
  ...
  const daeunTransitionText = (() => {
    ...
    const head = `${nextYear ? `${nextYear}년부터` : "다음 대운에"} 대운이 ${_nextLabelClean}로 바뀌어. 대운이 바뀌는 전후 2~3년은 인생의 환절기라, 겉으론 멀쩡해 보여도 속에서 판이 크게 흔들려. `
    const body = dir === "up"
  ...
  const _daeunSaeunPeak = (() => {
    ...
    const years = goods.map(y => {
      ...
      return { year: y.year, score: y.score, area: pick?.[0] || "", msg: _AREA_MSG[pick?.[0]] || "여러 영역이 고르게 받쳐줘서, 뭘 벌여도 손해가 적은 해야." }
    })
  ...
  const _sajuCommon3 = [
    ...
    `여기에 일간까지 더하면, 타고난 기질이 훨씬 또렷하게 드러나.`,
  ].join(" ")
  const _take2Sent = (str) => (str || "").split(/(?<=[.?!])\s+/).filter(Boolean).slice(0, 2)
  const _std2 = _take2Sent(iljuDescStd)
  const _mid2 = _take2Sent(iljuDescMid)
  ...
  const sajuSummaryText = `정리하면 이 사주는 십성으로는 ${_sibsongTop1?.label || "특정 기운"}이 가장 강하고, 일주 자리엔 ${(_sinsalDayItem?.name || "").replace(/\(.*\)/, "") || "고유한 신살"}이 있어. 운성으로는 ${(_unseongDayItem?.stage || "").replace(/\(.*\)/, "") || "고유한 단계"}에 걸려 있는데, 이건 지금 기운이 자라는 중인지 무르익었는지를 보여주는 지표야. 십성·신살·운성이 다 같은 결을 가리키고 있으니, 이 흐름을 거스르지 말고 타고 가는 게 맞아. 이 세 가지를 따로따로 보지 말고 하나의 이야기로 읽으면, 이 사주가 지금 어디로 가고 있는지 훨씬 선명해져.`
