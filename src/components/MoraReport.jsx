...
export default function MoraReport({ d, onHome, onSavePDF, pdfLoading, pdfMode, parentAstroAI, setParentAstroAI, parentTarotAI, setParentTarotAI }) {
 ...
 const personaHook = `${_persoCore} ${_punch}`
  ...
  const childhoodText = (() => {
    ...
    let base
    if (["편인","정인"].includes(yj)) base = "유년기에 보살핌과 관심을 넉넉히 받고 자란 편이야. 부모나 조부모의 사랑이 두터워 그 안정감이 지금의 정서적 뿌리가 됐어. 다만 기대와 간섭도 함께 컸을 수 있어."
    else if (["편관","정관"].includes(yj)) base = "유년기에 규율과 기대 속에서 자란 편이야. 엄한 분위기가 일찍 철들게 만들었고, 어른스러운 만큼 마음껏 응석 부릴 틈은 적었을 수 있어."
    else if (["편재","정재"].includes(yj)) base = "유년기에 현실적이고 활기찬 가정에서 자란 편이야. 부모가 바쁘게 살림을 꾸리는 걸 보며 생활력과 눈치를 일찍 익혔어."
    else if (["식신","상관"].includes(yj)) base = "유년기에 자유롭고 표현이 허용되는 분위기에서 자란 편이야. 감정을 드러내도 받아주는 환경이 지금의 창의성과 표현력으로 이어졌어."
    else base = "유년기의 가정 분위기가 지금의 정서적 뿌리를 만들었어. 그 시절 부모와 맺은 관계 방식이, 지금 사람을 믿고 곁을 내주는 방식에 남아 있어."
    // 아버지(편재) — 존재감·거리
    const pj = d.sibsongAnalysis?.counts?.["편재"] || 0
    const father = pj >= 1
      ? " 아버지는 밖에서 부지런히 움직이며 생활을 책임진 분이었을 가능성이 커. 든든했지만 얼굴 맞댈 시간은 적어 정서적으로 조금 먼 느낌이 남았을 수 있어."
      : " 아버지를 뜻하는 기운이 뚜렷하지 않아, 일찍 거리가 생겼거나 존재감이 옅어서 그 빈자리를 스스로 채우며 자랐을 수 있어."
    // 어머니(인성) — 존재감·거리
    const mj = (d.sibsongAnalysis?.counts?.["정인"] || 0) + (d.sibsongAnalysis?.counts?.["편인"] || 0)
    const mother = mj >= 1
      ? " 어머니는 곁에서 정서적으로 감싸주는 분위기였을 가능성이 커. 세심한 관심이 때로 간섭처럼 느껴졌을 수도 있어."
      : " 어머니의 손길은 조금 옅었을 수 있어. 알아서 크도록 둔 편이라 정서적으로는 일찍 자립하는 법을 배웠어."
    // 오행 결핍 → 자란 환경 추론
    const env = missingOh.length
      ? ` 타고난 오행에 ${missingOh.map(k => OHK_KR[k]).join(", ")} 기운이 비어, 그 부분은 스스로 찾아 채우게 만든 환경이었어.`
      : " 오행이 고루 갖춰진 만큼 자란 환경도 한쪽으로 치우치지 않았어."
  ...
  const healthMindLifeText = "몸보다 마음이 먼저 지치는 사주야. 스트레스가 몸으로 내려오기 전에 마음을 비우는 루틴을 만들어. 명상이든 산책이든 혼자만의 회복 시간이 약보다 중요해. 마음이 편해야 몸도 따라서 편해지는 구조라, 감정을 방치하지 않는 게 가장 확실한 건강 관리야."
  ...
  const yukchinLoopText = pick5({
    관성: "관계에서 상대의 기대와 규칙에 나를 맞추는 패턴을 반복해. 가족 안에서 '잘해야 사랑받는다'고 익힌 각본이 어른이 된 지금도 따라와. 그 각본을 알아차리는 순간, 조건 없이도 사랑받을 자격이 있다는 걸 알게 돼. 애써 증명하지 않아도 곁에 남는 사람이 진짜라는 걸 배우는 게 이번 생의 과제야.",
    인성: "관계에서 받기보다 먼저 주고 희생하는 패턴을 반복해. 가족 안에서 '내가 챙겨야 한다'고 익힌 각본이 지금도 따라와. 그 각본을 알아차리는 순간, 나도 기대고 받아도 된다는 걸 배우게 돼. 주기만 하는 관계에서 받는 법을 익히는 게 이번 생의 숙제야.",
    식상: "관계에서 내가 앞장서 이끌고 분위기를 책임지는 패턴을 반복해. 가족 안에서 '내가 나서야 굴러간다'고 익힌 각본이 지금도 따라와. 그 각본을 내려놓는 순간, 남에게 맡기고 기대는 편안함도 알게 돼. 안 끌어도 관계는 굴러간다는 걸 믿는 연습이 필요해.",
    비겁: "관계에서 지지 않으려 하고 주도권을 두고 힘겨루기하는 패턴을 반복해. 가족 안에서 '내 몫은 내가 지킨다'고 익힌 각본이 지금도 따라와. 그 각본을 알아차리는 순간, 이겨야만 하는 게 아니란 걸 알게 돼. 힘을 빼고 그냥 함께 있는 법을 배우는 게 관건이야.",
    재성: "관계에서 상대를 챙기고 은근히 통제하려는 패턴을 반복해. 가족 안에서 '내가 관리해야 안심된다'고 익힌 각본이 지금도 따라와. 그 각본을 알아차리는 순간, 놓아줘도 관계가 무너지지 않는다는 걸 알게 돼. 통제하지 않아도 관계는 지켜진다는 믿음이 이번 생의 숙제야.",
  })
