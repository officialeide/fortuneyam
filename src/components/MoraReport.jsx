...
export default function MoraReport({ d, onHome, onSavePDF, pdfLoading, pdfMode, parentAstroAI, setParentAstroAI, parentTarotAI, setParentTarotAI }) {
  ...
  const wealthPatternText = pick5({
    ...
    관성: "능력은 있어. 인정도 받아. 근데 자리·직책이 서야 돈이 따라오는 구조야. 눈앞의 벌이보다 나를 인정해줄 판에 올라타는 게 먼저야.",
  ...
  const healthMentalText = pick5({
    관성: "책임과 압박을 스스로 짊어지다 긴장이 몸으로 오는 구조야. 어깨·목 결림이나 불면으로 나오기 쉬우니, 다 짊어지려는 마음을 내려놓는 게 정신 건강 관리야.",
    인성: "혼자 생각을 곱씹고 담아두는 게 많은 구조야. 표현하지 않으면 몸으로 나오니, 글쓰기나 가까운 사람한테 털어놓기, 혼자만의 회복 시간이 약이야.",
    식상: "에너지를 계속 밖으로 쏟다 방전되는 구조야. 신나게 달리다 갑자기 무기력이 오기 쉬우니, 의식적으로 멈추고 충전하는 리듬을 만드는 게 관리법이야.",
    비겁: "쉬지 않고 움직이려는 기질이라 몸을 혹사하기 쉬워. 멈추는 걸 못 견디는 게 함정이니, 강제로라도 쉬는 시간을 넣는 게 정신 건강의 열쇠야.",
    재성: "여러 일을 동시에 붙들다 과부하가 오는 구조야. 다 챙기려다 신경이 곤두서기 쉬우니, 우선순위를 정해 덜어내는 게 마음을 지키는 법이야.",
  })
  ...
  const childhoodText = (() => {
    ...
    const mother = mj >= 1
      ? " 어머니는 곁에서 정서적으로 감싸주는 분위기였을 가능성이 커. 세심하게 챙겨준 만큼 마음의 안정은 얻었지만, 그 관심이 때로 간섭처럼 느껴졌을 수도 있어."
      : " 반대로 어머니의 손길은 조금 옅었을 수 있어. 알아서 크도록 둔 편이라, 정서적으로는 일찍 자립하는 법을 배웠어."
    // 오행 결핍 → 자란 환경 추론
    const env = missingOh.length
      ? ` 타고난 오행에 ${missingOh.map(k => OHK_KR[k]).join(", ")} 기운이 비어, 자란 환경도 그 부분은 스스로 찾아 채우게 만든 쪽이었어.`
      : " 오행이 고루 갖춰진 만큼 자란 환경도 한쪽으로 크게 치우치지 않았어."
  ...
  const spousePalaceText = (() => {
    ...
    let core
    if (["편재","정재"].includes(ilji)) core = "배우자 자리에 재성이 앉아 있어. 현실적이고 생활력 있는 짝을 만날 가능성이 커. 알뜰하게 살림을 꾸리고 돈 관리에 밝은 상대라, 배우자가 재물운을 밀어주는 구조야. 결혼 후 경제적으로 안정되는 흐름이고, 배우자 덕에 기회가 들어오기도 해. 성격은 다정하고 사교적인 편이라 사람들과 두루 잘 지내지만, 현실 감각이 뚜렷한 만큼 가끔 계산이 빠르다고 느껴질 수도 있어."
    else if (["편관","정관"].includes(ilji)) core = "배우자 자리에 관성이 앉아 있어. 듬직하고 책임감 있는 짝, 사회적으로 인정받거나 반듯한 상대를 만날 가능성이 커. 원칙이 분명하고 나를 세워주는 사람이라, 배우자가 내 삶의 기둥이 되어주는 구조야. 다만 자기 기준이 확고해서 때로 엄격하거나 고집스럽게 느껴질 수 있어. 서로의 방식을 존중해주면 더없이 든든한 울타리가 되는 인연이야."
    else if (["편인","정인"].includes(ilji)) core = "배우자 자리에 인성이 앉아 있어. 따뜻하고 나를 챙겨주는 짝, 정신적으로 기댈 수 있는 상대를 만날 가능성이 커. 배려심 깊고 생각이 많은 사람이라, 힘들 때 마음의 안식처가 되어줘. 다만 표현이 은근하고 속을 잘 안 내비쳐서, 애정을 눈에 보이게 확인하고 싶은 사람에겐 답답할 수도 있어. 오래 곁을 지키는 편안한 인연이야."
    else if (["식신","상관"].includes(ilji)) core = "배우자 자리에 식상이 앉아 있어. 표현이 풍부하고 함께 있으면 즐거운 짝을 만날 가능성이 커. 재치 있고 감각적이라 삶에 활기를 불어넣는 상대야. 다만 자유로운 기질이 강해서 얽매이는 걸 싫어하니, 서로의 공간을 존중해야 오래가. 감정 표현이 솔직한 만큼 티격태격도 있지만, 그만큼 정도 빨리 쌓이는 인연이야."
    else core = "배우자 자리에 비겁이 앉아 있어. 나와 비슷한 기질의 짝, 친구처럼 편안한 상대를 만날 가능성이 커. 대등하게 손잡고 가는 동반자형이라 말이 잘 통하고 함께 성장하는 구조야. 다만 둘 다 자기 색이 강하면 주도권을 두고 부딪힐 수 있으니, 서로의 영역을 인정해주는 게 오래가는 비결이야."
  ...
  const drainPatternText = pick5({
    관성: "남의 기대에 부응하려 무리하게 떠안다 기가 빠져. '거절하면 실망시킨다'는 부담이 나를 갉으니, 못 하는 건 못 한다고 말하는 게 기를 지키는 법이야.",
    인성: "주변의 감정을 쉽게 흡수하는 구조야. 힘든 사람 곁에 있으면 나도 같이 가라앉으니, 받는 것 없이 주기만 하는 관계는 일찍 정리해야 해.",
    식상: "재밌으면 끝을 안 보고 쏟아붓다 소진돼. 신나서 달릴 때 스스로 브레이크를 못 거는 게 함정이니, 에너지 총량을 정해두고 쓰는 게 필요해.",
    비겁: "남의 부탁을 거절 못 하고 내 몫 아닌 것까지 짊어지다 지쳐. 다 떠안는 패턴을 알아채고 선을 긋는 게 먼저야.",
    재성: "여러 사람·여러 일을 다 관리하려다 진이 빠져. 손 안의 걸 다 붙들려는 마음을 놓고, 놓아도 되는 건 놓는 연습이 기를 지켜.",
  })
  ...
  const reomulType = _jaeStruct === "편재"
    ...
    : _jaeStruct === "정재"
    ? `돈이 또박또박 들어오는 구조야. 한 방보다 꾸준히 쌓을 때 빛나는 사주라 월급·고정수입 같은 안정적인 흐름이 잘 맞아. 조급하게 큰 걸 노리다 페이스를 잃으니, 복리처럼 천천히 불리는 게 정답이야.`
    : _jaeStruct === "과다"
    ? `돈 기회는 넘치는데 그만큼 새는 구조야. 벌 데도 많지만 쓸 데·나눌 데도 많아서 늘 바쁜데 안 남아. 다 붙잡으려 하면 전부 흩어지니, 핵심 파이프 하나만 남기고 정리하는 게 돈을 쥐는 법이야.`
    : _jaeStruct === "없음"
  ...
  const reomulInvest = _jaeStruct === "편재"
    ? "크게 베팅하고 크게 회수하는 스타일이 맞아. 감각은 좋은데 한 방에 몰빵하면 위험하니, 번 것의 일부는 반드시 안전자산에 묶어둬."
    : _jaeStruct === "정재"
    ? "차곡차곡 적립하고 복리로 굴리는 안정형이 맞아. 한 번에 크게 노리다 페이스 잃으니, 지루해도 꾸준한 게 정답이야."
    : _jaeStruct === "과다"
    ? "여러 곳에 분산하는 감각은 좋은데 관리가 관건이야. 벌인 걸 다 못 쫓으면 전부 새니, 핵심만 남기고 정리하는 게 이득이야."
    : _jaeStruct === "없음"
    ? "투기보다 저축과 현금흐름이 먼저인 구조야. 무리한 투자는 이 사주엔 독이니, 지키는 재테크부터 다지는 게 맞아."
    : isSingang
  ...
  const transitionPrepText = pick5({
    관성: "대운 전환기엔 무리하게 새 책임을 떠안기보다, 지금 자리를 정리하고 다음 판을 볼 준비를 해. 조급하게 밀면 탈이 나니 흐름을 읽고 미리 방향만 틀어둬.",
    인성: "대운 전환기는 배우고 채우기 좋은 때야. 큰일을 벌이기보다 실력·자격을 쌓아 다음 흐름에 올라탈 준비를 하면, 전환기가 도약의 발판이 돼.",
    식상: "대운 전환기엔 벌여둔 걸 정리하고 진짜 하고 싶은 하나에 집중해. 다 붙들면 전부 어정쩡해지니, 흐름이 바뀔 때 과감히 선택하는 게 관건이야.",
    비겁: "대운 전환기엔 혼자 다 짊어지려 말고 힘을 아껴. 변화를 두려워만 하면 기회도 지나가니, 믿을 사람과 손잡고 흐름을 함께 넘는 게 나아.",
    재성: "대운 전환기엔 벌인 판을 슬림하게 정리하고 다음 기회를 노려. 흐름을 읽고 미리 방향을 틀어두면 전환기가 오히려 크게 버는 발판이 돼.",
  })
  ...
  const companyFitText2 = (() => {
    ...
    if (joinFit && elFit && elFit !== "neutral") {
      if (elFit === "good" && joinFit === "good") verdict = `자리(업종)가 나를 살리는 ${k} 기운이라 잘 맞고, 들어온 때의 궁합도 좋아. 이 회사에선 실력이 제대로 인정받고 오래 갈수록 빛나는 궁합이야.`
      else if (elFit === "good" && joinFit === "weak") verdict = `자리(업종)는 나를 살리는 ${k} 기운이라 잘 맞는데, 들어온 시기의 궁합이 살짝 아쉬워. 그래서 처음엔 인정이 더디게 느껴져도, 버티며 자리를 지킬수록 이 업이 나를 키워주는 흐름이야.`
      else if (elFit === "bad" && joinFit === "good") verdict = `들어온 때는 나쁘지 않은데, 업종 자체가 내 기신인 ${k} 기운이라 결이 어긋나. 조직에선 인정받아도 이 분야가 계속 기운을 갉으니, 여기서 실력을 쌓아 더 맞는 자리로 옮겨갈 발판으로 삼는 게 현명해.`
      else verdict = `솔직히 자리(업종)도 들어온 때도 딱 맞물리는 궁합은 아니야. ${k} 기운이 나와 어긋나는 데다 시기도 아쉬워서, 여기서 다 채우려 하기보다 실력과 경력을 쌓는 발판으로 삼고 다음 흐름을 준비하는 게 나아.`
    } else if (elFit) {
      verdict = elFit === "good" ? `업종을 보면 이 회사는 나를 살리는 ${k} 기운이라, 오래 다닐수록 이 자리가 나를 키워주는 궁합이야.`
        : elFit === "bad" ? `업종을 보면 이 회사는 내 기신인 ${k} 기운이라, 조건이 좋아 보여도 은근히 기운이 새는 자리야. 오래 머물 곳인지 신중히 봐.`
        : `업종을 보면 이 회사는 ${k} 기운인데 나와 상생도 상극도 아닌 중립이라, 궁합보다 실무 조건과 사람으로 판단하면 돼.`
      if (hasJoin) verdict = `${_joinYear}년에 들어간 회사야. ` + verdict
    } else if (joinFit) {
      verdict = `${_joinYear}년에 들어간 회사야. ${joinFit === "good" ? "입사한 해의 기운이 이 사주와 맞물려서, 이 조직에서 자리를 잡고 인정받을 수 있는 궁합이야. 버티면 열매가 있어." : "입사한 해의 궁합이 딱 맞물리진 않아. 여기서 다 채우려 하기보다 실력을 쌓는 발판으로 삼는 게 현명해."}`
    }
    let ageTail = ""
    if (_foundYear) {
      const age = (new Date().getFullYear()) - _foundYear
      ageTail = ` 참고로 ${_foundYear}년생 회사라 ${age < 5 ? "아직 어린 조직이야. 함께 자리를 잡아가는 재미가 있는 대신 안정성은 덜해." : age < 15 ? "한창 성장하는 조직이라 체계와 기회가 함께 있어." : "충분히 자리 잡은 조직이라 안정과 체계는 있지만 변화 속도는 느릴 수 있어."}`
    }
  ...
  const interviewText = pick5({
    관성: "면접에선 '책임감 있고 신뢰 가는 사람' 이미지가 먹혀. 튀는 개성보다 안정감과 성실함을 앞세워. 하나를 끝까지 해낸 완수 경험을 강조하면 유리해.",
    식상: "면접에선 말솜씨와 순발력이 무기야. 짜인 답보다 상황에 맞게 풀어내는 힘이 강점이라, 대화형 면접이나 발표에서 빛나. 창의성과 표현력을 보여줘.",
    인성: "면접에선 차분함과 준비된 태도가 강점이야. 즉흥보다 미리 탄탄히 준비했을 때 실력이 나오니, 예상 질문을 충분히 연습하고 들어가면 합격률이 확 올라가.",
    재성: "면접에선 실전 감각과 성과로 승부해. 추상적인 포부보다 '무엇을 얼마나 만들어냈는가'를 숫자와 결과로 보여줄 때, 실무형이라는 인상이 강하게 박혀.",
    비겁: "면접에선 뚝심과 주도성이 강점이야. 주눅 들지 않는 태도와 '맡겨만 주면 해낸다'는 자신감이 먹히니, 위축되지 말고 당당하게 밀어붙여.",
  })
  ...
  const relSocialStyle = pick5({
    관성: "관계에서 책임지고 조율하는 쪽이야. 모임에서 총대 메고 챙기는 역할을 맡는데, 그 부담을 혼자 지다 지쳐. 다 감당하려 말고 나눌 줄도 알아야 해.",
    인성: "관계에서 받는 걸 어려워하는 쪽이야. 남 챙기는 건 잘하는데 정작 내 얘기는 잘 안 꺼내. 깊은 관계는 소수랑만 맺고, 그 소수한테는 뭐든 내주는 사람이야.",
    식상: "관계에서 분위기를 띄우고 이끄는 쪽이야. 말과 표현으로 사람을 끌어당기는데, 에너지를 다 쏟고 혼자 방전되기 쉬우니 회복 시간을 꼭 챙겨.",
    비겁: "관계에서 대등하게 어울리는 쪽이야. 친구는 많은데 주도권을 두고 은근히 겨루기도 해. 지지 않으려는 마음을 내려놓으면 관계가 훨씬 편해져.",
    재성: "관계에서 사람을 잘 관리하고 챙기는 쪽이야. 인맥을 넓게 쓰는 감각이 좋은데, 정 주는 만큼 통제하려 들면 멀어지니 적당한 거리가 필요해.",
  })
  ...
  const healthLifeText = pick5({
    관성: "긴장을 안고 사는 기질이라 몸이 늘 살짝 굳어 있어. 스트레스가 소화기나 근육 긴장으로 오기 쉬우니, 규칙적으로 몸을 푸는 루틴이 곧 보약이야.",
    인성: "타고난 체력이 넘치는 편은 아니야. 대신 무리만 안 하면 잔병 없이 오래 가는 구조라, 몸이 보내는 신호에 예민한 게 강점이야. 초기에 잡으면 큰 병으로 안 커져.",
    식상: "에너지를 몰아 쓰다 한 번에 방전되는 리듬이라, 몰입과 휴식의 낙차가 커. 달릴 때와 쉴 때를 미리 정해두는 규칙이 몸을 지켜.",
    비겁: "몸을 계속 쓰려 드는 기질이라 젊을 땐 체력으로 버텨. 근데 그게 독이야. 누적된 피로가 삼사십대에 터지기 쉬우니, 멈출 줄 아는 게 장수 비결이야.",
    재성: "여러 일을 벌이며 몸을 과하게 굴리는 편이야. 바쁠수록 끼니와 잠이 무너지기 쉬우니, 기본 리듬을 지키는 게 가장 큰 재테크이자 건강법이야.",
  })
  ...
  const yukchinLoopText = pick5({
    관성: "관계에서 상대의 기대와 규칙에 나를 맞추는 패턴을 반복해. 가족 안에서 '잘해야 사랑받는다'고 익힌 각본이 어른이 된 지금도 따라와. 그 각본을 알아차리는 순간, 조건 없이도 사랑받을 자격이 있다는 걸 알게 돼.",
    인성: "관계에서 받기보다 먼저 주고 희생하는 패턴을 반복해. 가족 안에서 '내가 챙겨야 한다'고 익힌 각본이 지금도 따라와. 그 각본을 알아차리는 순간, 나도 기대고 받아도 된다는 걸 배우게 돼.",
    식상: "관계에서 내가 앞장서 이끌고 분위기를 책임지는 패턴을 반복해. 가족 안에서 '내가 나서야 굴러간다'고 익힌 각본이 지금도 따라와. 그 각본을 내려놓는 순간, 남에게 맡기고 기대는 편안함도 알게 돼.",
    비겁: "관계에서 지지 않으려 하고 주도권을 두고 힘겨루기하는 패턴을 반복해. 가족 안에서 '내 몫은 내가 지킨다'고 익힌 각본이 지금도 따라와. 그 각본을 알아차리는 순간, 이겨야만 하는 게 아니란 걸 알게 돼.",
    재성: "관계에서 상대를 챙기고 은근히 통제하려는 패턴을 반복해. 가족 안에서 '내가 관리해야 안심된다'고 익힌 각본이 지금도 따라와. 그 각본을 알아차리는 순간, 놓아줘도 관계가 무너지지 않는다는 걸 알게 돼.",
  })
