...
function buildAstroPrompt(pillarsStr, ilganKo, ilganHanja, birth){
  return `사주 명식: ${pillarsStr}. 일간(천간, 나 자신): ${ilganKo}(${ilganHanja}) — 이것이 일간이야. 생년월일: ${birth}.
...
반드시 ~이야, ~해 체로 작성해줘. 한자는 반드시 한글(한자) 형식으로 병기해줘. 일간 한자는 반드시 ${ilganHanja}로만 표기해줘.
...
반드시 ~이야, ~해 체(언니체)로 작성해줘. 한 문장은 어떤 방식으로 성취하는지, 한 문장은 어떤 환경에서 빛나는지.
...
반드시 ~이야, ~해 체(언니체). "${data.name}님"으로 지칭. 이모지·마크다운 금지.
...
[출력 형식 — 반드시 준수]
- 말투: ~해요/~이야 친근한 존댓말. "${data.name}님"으로 지칭. 한자·이모지·마크다운 금지.
