...
export async function findCachedReport(formInput) {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      id,
      reports (id, full_data_json, prompt_version, created_at)
    `)
    .eq('birth_date', birth_date)
    .eq('birth_time', birth_time)
    .eq('gender', gender)
  ...
  const matched = allReports
    .filter(r => r.prompt_version === PROMPT_VERSION)
    .sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0]

  return matched ? matched.full_data_json : null
}
